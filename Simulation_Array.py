    ...
    for i in range(x-1):
        ...
        if zw:
            if randint(1,2)==1:
                ...
                axis=norm(rotate(axis,radians([60,180,300][randint(0,2)]),vector(0,0,1)))
        elif winkel:
...
def annaeherung(x,zl=False,zw=False,laengen=[],winkel=[],gn=360,sim=False):
    global P1,P2
    P1=vector(0,0,0)
    P2=vector(0,10,0)
    n=0
    ...
    if x<6:
        while n==0 or mag(a[-1]-P1)+mag(a[-2]-P2)>36./gn:
            ...
            a=drehezufaellig(a,[randint(30,330) if randint(1,2)==1 else pi/3 for i in range(x)])
            a,z,minabstaende=version1(a)
    else:
        while n==0 or mag(a[-1]-P1)+mag(a[-2]-P2)>36./gn:
            n+=1
            a=kette(x,zl,zw,laengen,winkel)
            a=drehezufaellig(a,[randint(30,330) if randint(1,2)==1 else [pi,2*pi/3][randint(0,1)] for i in range(x)])
            a,z,minabstaende=version1(a)
    if sim:
...
##        self.Bind(wx.EVT_NOTEBOOK_PAGE_CHANGED,self.notebookpagechanged,self.vpythonnotebook)
    ...
    def simulation(self,evt):
        ...
        if self.r2.GetSelection()==0:
            ...
            self.setwindow()
            print "Anzahl Versuche     :",n
            print
            print "Anzahl Annäherungen :",z
