...
from math import *
from visual import pi,cos,sin,sphere,cylinder,cone,curve,cross,rotate,vector,display,arcsin,diff_angle,norm,mag,absolute,arange,frame,label,proj,materials
...
from Simulation_Array import annaeherung
...
xx = atomklasse(0,(1,1,1),-1,0,0.01,0)
for i in atomlist:
...
def Visual(x,position=vector(0,0,0),axxxis=vector(0,1,0),winkel=0,pos=[],oldaxis=vector(0,0,-1),erstes=False):
    nzz=[]
    global lokanten,cislist,translist,AllIn,YL,Yl1,Yl2,AllAtoms,MFB,sizeC,sizeB,sizeCprop,lengthB,AllIn2,Vyl,valenzep2,nachdrehen,komplexvalenz
    if erstes:
        ...
        x2=x[0]
        AllAtoms.append(sphere(pos=position,color=atominfo[0],radius=atominfo[1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
    else:
            ...
            if x[i][1][2]!=0:
                if type(x[i][1][2])==type("cis"):
                    ...
                    if abs(x[i][1][2])!=pi and x[i][1][2]!=0:
                        konfigdic2[i-1]=1
                    if x[i][1][2]!=0 and not(x[1][1]==2 and abs(abs(x[i][1][2])-pi)>.1):
                        winkel=winkel+x[i][1][2]#+2*pi/3*(i-2)
        else:
            konfig.append(x[i][1])
    pos=atom(x[0],saettigungsbeilage,position,axxxis,konfig,winkel,alpha=alpha,ct=cistrans,oldaxis=oldaxis,konfigdic2=konfigdic2,cyclolaengen=laengen)
    pos2=pos[1]
    valenzen2=pos[2]
    testebitte=pos[3]
    pos=pos[0]
    ...
    for i in range(len(x[1:])):
        ...
        if x[i+1][1][2]==10*pi:
            drehid=0
            for i1 in x[i+1][1]:
                if i1.__class__==str and i1.startswith("n"):
                    drehid=eval(i1[1:])
            while len(nachdrehen)<drehid+1:
                nachdrehen.append(0)
            nachdrehen[drehid]=len(MFB)-len(x)+i+1
            ...
            if zahl in AllIn2[i2]:
                ...
                if len(pseudoAtome)==2:
                    ...
                    if abs(wunschort-pseudoAtome[0])>0.1:
                        ...
                        drehpunkt=proj(pseudoAtome[0]-position,achse)+position
                        winkel=diff_angle2(davor-drehpunkt,pseudoAtome[0]-drehpunkt)
                        drehzahl=allenachzahl(zahlen,MFB[nachdrehen[i]][1]-1)
                        if mag(rotate(pseudoAtome[0]-drehpunkt,winkel,achse)+drehpunkt-davor)>0.1:
                            achse*=-1
                        drehenachzahl(drehzahl,winkel,achse,drehpunkt,pseudoAtome[0])
                    pseudoAtome=[]
...
def allenachzahl(x,zahl):
    if x[0]==zahl:
        ...
        for i in x[1:]:
            ...
            if a:
                return a
def drehenachzahl(x,winkel,achse,drehpunkt,pseudo):
    global AllIn
    for i in x[1:]:
        AllIn[i[0]][0]=rotate(AllIn[i[0]][0]-drehpunkt,winkel,achse)+drehpunkt
        drehenachzahl(i,winkel,achse,drehpunkt,pseudo)
def drehmesser(a,b):
    if a==-1 and b==-1:
        return -1
    if a==-2 and b==-2:
...
def mehrfachbindung(pos,axis,art,atome,pointon=0,keilstrich=False):
    global sizeC,sizeB,sizeCprop,opacitychosen2,bindungsg2
    lengthB=mag(pos-axis)
    f=frame()
    enatom1=en(atome[0])
    enatom2=en(atome[1])
    ...
    if art==2:
        ...
        if keilstrich and abs(enatom1-enatom2)>0.4:
            if enatom1-enatom2>0.4:
                n1=cone(frame=f,pos=(0,lengthB/10,0),axis=(lengthB,0,0),radius=sizeB*3/4,color=atom1[0],material=materials.wood)
                n2=cone(frame=f,pos=(0,-lengthB/10,0),axis=(lengthB,0,0),radius=sizeB*3/4,color=atom1[0],material=materials.wood)
            else:
                n1=cone(frame=f,pos=(lengthB,lengthB/10,0),axis=(-lengthB,0,0),radius=sizeB*3/4,color=atom2[0],material=materials.wood)
                n2=cone(frame=f,pos=(lengthB,-lengthB/10,0),axis=(-lengthB,0,0),radius=sizeB*3/4,color=atom2[0],material=materials.wood)
        else:
            n1=curve(frame=f,x=arange(-lengthB/2.,-lengthB/2.+laenge/2.+atom1[1]*sizeC+sizeCprop+verbindungsstueck,0.5)+lengthB/2.,radius=sizeB,color=atom1[0],material=materials.wood)
            n1.y=((lengthB**2-(n1.x-lengthB/2.)**2)**.5-sin(pi/3)*lengthB)*2
            n1.z=n1.x*0
            n3=curve(frame=f,x=arange(lengthB/2.,lengthB/2.-laenge/2.-atom1[1]*sizeC-sizeCprop-verbindungsstueck,-0.5)+lengthB/2.,radius=sizeB,color=atom2[0],material=materials.wood)
            n3.y=n1.y
            n3.z=n1.x*0
            if n3.x[-1]!=n1.x[-1]:
                ...
                n3.y=n1.y
                n3.z=n1.x*0
            n2=curve(frame=f,x=arange(-lengthB/2.,-lengthB/2.+laenge/2.+atom1[1]*sizeC+sizeCprop+verbindungsstueck,0.5)+lengthB/2.,radius=sizeB,color=atom1[0],material=materials.wood)
            n2.y=-n1.y
            n2.z=n1.x*0
            n4=curve(frame=f,x=arange(lengthB/2.,lengthB/2.-laenge/2.-atom1[1]*sizeC-sizeCprop-verbindungsstueck,-0.5)+lengthB/2.,radius=sizeB,color=atom2[0],material=materials.wood)
            n4.y=-n1.y
            n4.z=n2.x*0
    elif art==3:
        axis=axis-pos
        if keilstrich and abs(enatom1-enatom2)>0.4:
            if enatom1-enatom2>0.4:
                n1=cone(frame=f,pos=(0,lengthB/10,0),axis=(lengthB,0,0),radius=sizeB*3/4,color=atom1[0],material=materials.wood)
                n2=cone(frame=f,pos=(0,-cos(pi/3)*lengthB/10,-sin(pi/3)*lengthB/10),axis=(lengthB,0,0),radius=sizeB*3/4,color=atom1[0],material=materials.wood)
                n3=cone(frame=f,pos=(0,-cos(pi/3)*lengthB/10,sin(pi/3)*lengthB/10),axis=(lengthB,0,0),radius=sizeB*3/4,color=atom1[0],material=materials.wood)
            else:
                n1=cone(frame=f,pos=(lengthB,lengthB/10,0),axis=(-lengthB,0,0),radius=sizeB*3/4,color=atom2[0],material=materials.wood)
                n2=cone(frame=f,pos=(lengthB,-cos(pi/3)*lengthB/10,sin(pi/3)*lengthB/10),axis=(-lengthB,0,0),radius=sizeB*3/4,color=atom2[0],material=materials.wood)
                n3=cone(frame=f,pos=(lengthB,-cos(pi/3)*lengthB/10,-sin(pi/3)*lengthB/10),axis=(-lengthB,0,0),radius=sizeB*3/4,color=atom2[0],material=materials.wood)
        else:
            n1=curve(frame=f,x=arange(-lengthB/2.,-lengthB/2.+laenge/2.+atom1[1]*sizeC+sizeCprop+verbindungsstueck,0.5)+lengthB/2.,radius=sizeB,color=atom1[0],material=materials.wood)
            n1.y=((lengthB**2-(n1.x-lengthB/2.)**2)**.5-sin(pi/3)*lengthB)*2
            n1.z=n1.x*0
            n4=curve(frame=f,x=arange(lengthB/2.,lengthB/2.-laenge/2.-atom1[1]*sizeC-sizeCprop-verbindungsstueck,-0.5)+lengthB/2.,radius=sizeB,color=atom2[0],material=materials.wood)
            n4.y=((lengthB**2-(n1.x-lengthB/2.)**2)**.5-sin(pi/3)*lengthB)*2
            n4.z=n1.x*0
            if n4.x[-1]!=n1.x[-1]:
                n1.visible=False
                n4.visible=False
            ...
            n2.y=-n1.y*sin(pi/6.)
            n2.z=-n1.y*cos(pi/6.)
            n3=curve(frame=f,x=arange(-lengthB/2.,-lengthB/2.+laenge/2.+atom1[1]*sizeC+sizeCprop+verbindungsstueck,0.5)+lengthB/2.,radius=sizeB,color=atom1[0],material=materials.wood)
            n3.y=-n1.y*sin(pi/6.)
            n3.z=n1.y*cos(pi/6.)
            n5=curve(frame=f,x=arange(lengthB/2.,lengthB/2.-laenge/2.-atom1[1]*sizeC-sizeCprop-verbindungsstueck,-0.5)+lengthB/2.,radius=sizeB,color=atom2[0],material=materials.wood)
            n5.y=-n1.y*sin(pi/6.)
            n5.z=-n1.y*cos(pi/6.)
            n6=curve(frame=f,x=arange(lengthB/2.,lengthB/2.-laenge/2.-atom1[1]*sizeC-sizeCprop-verbindungsstueck,-0.5)+lengthB/2.,radius=sizeB,color=atom2[0],material=materials.wood)
            n6.y=-n1.y*sin(pi/6.)
            n6.z=n1.y*cos(pi/6.)
    elif art==1:
        if bindungsg2:
        ...
        if keilstrich and abs(enatom1-enatom2)>0.4:
            if enatom1-enatom2>0.4:
                n1=cone(pos=pos+plus1,axis=axis-pos,radius=sizeB*1.5,color=atom1[0],material=materials.wood)
                n2=None
            else:
                n1=cone(pos=axis-plus2,axis=pos-axis,radius=sizeB*1.5,color=atom2[0],material=materials.wood)
                n2=None
        else:
            n1=cylinder(pos=pos+plus1,axis=norm(axis-pos)*(laenge/2.+atom1[1]*sizeC+sizeCprop)-plus1,radius=sizeB,color=atom1[0],opacity=opacitychosen2,material=materials.wood)
            n2=cylinder(pos=axis+plus2,axis=norm(pos-axis)*(laenge/2.+atom2[1]*sizeC+sizeCprop)-plus2,radius=sizeB,color=atom2[0],opacity=opacitychosen2,material=materials.wood)
    if art==1:
        f=[n1,n2]
    else:
        f.axis=axis
        f.pos=pos
    return f
def atom(natom,saettigungsbeilage,posn,axxis,konfigdic,winkel=0,alpha=[],p=0,ct=0,oldaxis=vector(0,0,-1),konfigdic2=[0,0,0,0,0,0,0,0],cyclolaengen=[0,0,0,0,0,0,0,0]):
        global MFBausrichtung,AllIn,AllIn2,AllAtoms,MFB,sizeC,sizeB,sizeCprop,lengthB,valenzep2,opacitychosen,lokanten,wasserstoffonoff,bindungslaengen
        ...
        for i in range(len(saettigungsbeilage)):
            if saettigungsbeilage[i]==0:
                atominfo2.append(0)
            elif "@" in saettigungsbeilage[i]:
                lokanten.append("")
                atominfo2.append(([0,0,0],0,0,0))
            else:
                lokanten.append(mitzahlen(saettigungsbeilage[i]))
                saettigungsbeilage[i]=ohnezahlen(saettigungsbeilage[i])
                atominfo2.append(atom_color(saettigungsbeilage[i]))
            if wasserstoffonoff and cyclolaengen[i]:
                if natom in bindungslaengen[konfigdic[i]] and saettigungsbeilage[i] in bindungslaengen[konfigdic[i]][natom]:
                    laengen.append(bindungslaengen[konfigdic[i]][natom][saettigungsbeilage[i]]/154.)
                elif saettigungsbeilage[i] in bindungslaengen[konfigdic[i]] and natom in bindungslaengen[konfigdic[i]][saettigungsbeilage[i]]:
                    laengen.append(bindungslaengen[konfigdic[i]][saettigungsbeilage[i]][natom]/154.)
                else:
                    laengen.append(1)
            else:
                laengen.append(1)
        ...
        if atominfo2[0]!=0:
            AllAtoms.append(sphere(pos=pos[0],color=atominfo2[0][0],radius=atominfo2[0][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
        if len(konfigdic)==4 or (konfigdic==[1,1,1] and  valenzen>4) or (konfigdic==[1,1] and valenzen>5) or konfigdic==[1]:
            ...
            if len(konfigdic)==1:
                ...
                AllAtoms.append(sphere(pos=pos[1],color=atominfo2[1][0],radius=atominfo2[1][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
            if len(konfigdic)in(1,2):
                valenzep.append([(pos[2]-posn)/4.+posn,-2])
            else:
                AllAtoms.append(sphere(pos=pos[2],color=atominfo2[2][0],radius=atominfo2[2][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
            if len(konfigdic)!=4:
                valenzep.append([(pos[3]-posn)/4.+posn,-2])
                if len(konfigdic)==3:
                   testebitte.append([pos[2]-posn+pos[3],pos[2]])
            else:
                AllAtoms.append(sphere(pos=pos[3],color=atominfo2[3][0],radius=atominfo2[3][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
                ...
                AllAtoms.append(sphere(pos=pos[2],color=atominfo2[2][0],radius=atominfo2[2][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
                AllAtoms.append(sphere(pos=pos[3],color=atominfo2[3][0],radius=atominfo2[3][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
                AllAtoms.append(sphere(pos=pos[4],color=atominfo2[4][0],radius=atominfo2[4][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
        elif len(konfigdic)==6:
                pos[5]=posn+rotate(rotate((0,-lengthB*laengen[1],0),winkel,(0,1,0)),drehwinkel,achse)
                pos[4]=posn+rotate(rotate((-lengthB*laengen[2],0,0),winkel,(0,1,0)),drehwinkel,achse)
                pos[2]=posn+rotate(rotate((lengthB*laengen[3],0,0),winkel,(0,1,0)),drehwinkel,achse)
                pos[1]=posn+rotate(rotate((0,0,lengthB*laengen[4]),winkel,(0,1,0)),drehwinkel,achse)
                pos[3]=posn+rotate(rotate((0,0,-lengthB*laengen[5]),winkel,(0,1,0)),drehwinkel,achse)
                AllAtoms.append(sphere(pos=pos[1],color=atominfo2[1][0],radius=atominfo2[1][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
                AllAtoms.append(sphere(pos=pos[2],color=atominfo2[2][0],radius=atominfo2[2][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
                AllAtoms.append(sphere(pos=pos[3],color=atominfo2[3][0],radius=atominfo2[3][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
                AllAtoms.append(sphere(pos=pos[4],color=atominfo2[4][0],radius=atominfo2[4][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
                AllAtoms.append(sphere(pos=pos[5],color=atominfo2[5][0],radius=atominfo2[5][1]*sizeC+sizeCprop,opacity=opacitychosen,material=materials.wood))
...
global csubs,greek1,greek2,zuckerpraefixe,bindungslaengen,praesub,csubskeys
csubs={"fluora":["F",1],"chlora":["Cl",1],"broma":["Br",1],"ioda":["I",1],"mercura":["Hg",2],"stanna":["Sn",4],"germa":["Ge",4],"bisma":["Bi",3],"phospha":["P",3],"selena":["Se",2],"tellura":["Te",2],"arsa":["As",3],"stiba":["Sb",3],"plumba":["Pb",4],"aza":["N",3],"oxa":["O",2],"thia":["S",2],"sila":["Si",4],"bora":["B",3],"oxonia":["O",3],"thionia":["S",3],"azonia":["N",4]}
csubskeys=csubs.keys()
csubskeys.sort()
bindungslaengen=[[],{"C":{"H":107,"Be":193,"Mg":207,"B":156,"Al":224,"In":216,"C":154,"Si":186,"Sn":214,"Pb":229,"N":147,"P":187,"As":198,"Sb":220,"Bi":230,"O":143,"S":181,"Cr":192,"Se":198,"Te":205,"Mo":208,"W":206,"F":134,"Cl":176,"Br":193,"I":213},"N":{"H":100.5},"O":{"H":96.5},"Si":{"F":160}},{"C":{"C":135,"O":122,"N":127},"N":{"N":125}},{"C":{"C":120,"N":115},"N":{"N":110}}]
...
praesub={"thio":"S","seleno":"Se","telluro":"Te","polonio":"Po"}
...
multiid=0
class unterGruppen(list):
    def __init__(self,Gruppe):
        self=[]
    def unterordnen(self,objekt):
        self.append(objekt)
class Endungsdic(dict):
    def __init__(self,Endungen):
        self={}
    def dazu(self,Endung,stelle=0):
        if Endung.__class__ in (Endungsdic,dict):
            for i in Endung.keys():
                for i2 in Endung[i]:
                    self.dazu(i2,i)
        elif stelle in self:
            self[stelle].append(Endung)
        else:
            self[stelle]=[Endung]
class System:
    def __init__(self,Laenge,Endungen={},Bezeichnungen=[],sicherzugeordnete=[],Id=1,Lokant=1):
        self.Laenge=Laenge
        self.Endungen=Endungsdic({})
        self.Endungen.dazu(Endungen)
        self.Bezeichnungen=Bezeichnungen
        self.Untergruppen=unterGruppen([])
        for i in sicherzugeordnete:
            self.Untergruppen.unterordnen(i)
        self.Lokant=Lokant
        self.id=Id
        self.Radikal2=[]
        for i1 in self.Endungen.keys():
            for i2 in ("ylen","ylin","yliden","ylidin","yl","oyl","thioyl","oxy","azo","o(","azano","azeno"):
                if i2 in self.Endungen[i1]:
                    self.Radikal2.append(i2)
    def isRadikal(self):
        if "bi" in self.Bezeichnungen or "poly" in self.Bezeichnungen:
            return False
        self.Radikal2=[]
        for i1 in self.Endungen.keys():
            for i2 in ("ylen","ylin","yliden","ylidin","yl","oyl","thioyl","oxy","azo","o(","azano","azeno"):
        ...
        if self.Radikal2:
            return True
    def Radikalloesch(self):
        for i1 in self.Endungen.keys():
            for i2 in ("ylen","ylin","yliden","ylidin","yl","oyl","thioyl","oxy","azo","o(","azano","azeno"):
                if i2 in self.Endungen[i1]:
                    self.Endungen[i1].remove(i2)
        self.isRadikal()
    ...
    def inEndungen(self,parameter,boolean=True):
        indrin=[]
    ...
    def sucheVEP(self,speziell=True):
        a=analysezusyntax(self)
        if eval("symbol"+ohnezahlen(a[0])).valenz-zusammenzaehler(a)>1:
            return a
        a=sucheVEP(a,speziell)[0]
        a=Chiralitaetszentrenordner(a)
        a.insert(1,[0,1,0])
        return a
class Praefix:
    ...
    def adoptieren(self,parent):
        self.parent=parent
class Suffix:
    def __init__(self,Name,Lokant=1,Bezeichnungen=[],sichereBindungen=[1],Id=1,sicherzugeordnete=[]):
    ...
    def praefixconvert(self):
        self.Beschreibung=str(self.Beschreibung)
        gefunden="+"
        for i in ("+++","++","+"):
            if i in self.Beschreibung:
                a=self.Beschreibung.index("'"+i+"'")
                a=self.Beschreibung[:a]+"['"+i+"*'"+",[0,"+str(i.count("+"))+",0]]"+self.Beschreibung[a+2+len(i):]
                a=Chiralitaetszentrenordner(eval(a))
                self.Beschreibung=eval(str(a[1]).replace("'+'","['H',[0,1,0]]").replace("'++'","['H',[0,1,0]],['H',[0,1,0]]").replace("'+++'","['H',[0,1,0]],['H',[0,1,0]],['H',[0,1,0]]"))
                break
        self.csub=False
def analyse(x):
    global csubs,snl,zuckerpraefixe,greek1,cyc,greek2,snl,mi,praelist,fuell,fehlerapp,cyc2,multiid
    cyc2=0
    snl=[]
    praelist=[]
    fuell=[]
    ...
    if x.startswith("(") and sonderz(x)[0]=="":
            if x[0]=="(":
                ...
                if endstemp[0]!=[]:
                    if endstemp[0][0][0] in ("ylen","ylin","yliden","ylidin","yl","oyl","thioyl","oxy","azo","azano","azeno"):
            ...
            if temp[0] != "" or temp_hwp[0]!=[]:
                ...
                if not temp_hwp[0]:
                    ...
                    if gw in ("#tria","#tetra") and orstartswith(x,["asil","asil","agerman","astann","aplumb","astib"]):
                        x=x[1:]
                    if not gw.startswith("#") and not gw in ("mono","dia","tria","tetra") or orstartswith(x,["sil","asil","azan","azen","german","stann","plumb","stib"]):
                        if x.startswith("asil"):
                            x=x[1:]
                        endscoefi=[]
                        ncgw=ncalkan(gw)
                        if coefi and not "coeftest" in trivials:
                            for i in coefi[-1]:
                                if i.__class__==str:
                                    continue
                                if i>ncgw:
                    ...
                    for i in range(len(coefi[-1])):
                        ...
                        for i1 in trivials:
                            if i1.startswith("lambda"):
                                trivialsdazu.append(i1)
                                break
                        hwppraes.append(Praefix(temp_hwp[1][i],coefi[-1][i],[multiid]+trivialsdazu,Id=multiid))
                ...
                if endsilben!=[]:
                    ...
                    for i in coefi[-1]:
                        ...
                        if trivials4:
                            trivials4=[trivials4]
                        ausgabe.append(System(ncalkan(gw),end,trivials+trivials3+trivials4,Lokant=i,Id=multiid,sicherzugeordnete=hwppraes))
                    coefi=coefi[:-1]
                    endsilben=[]
                    trivials=[]
                continue
                    ...
                    if len(trytemp[0])+1>len(temp[0]):
                        ...
                        if coefi==[]:
                            ...
                            if not "per" in trivials:
                                trivials.append("coeftest")
                        multiid+=1
                        if len(coefi[-1])!=1 and not weg:
                            if "+" in praefix[praefixe]:
                                ausgabe.append(Praefix(praefixe,coefi[-1],trivials,Id=multiid))
                        else:
                            for i in coefi[-1]:
                                trivials3=[]
                                if i in trivials2.keys():
                                    for i1 in trivials2[i]:
                                        ...
                                        trivials3.append(i1)
                                        trivials2[i]=trivials2[i][1:]
                                ausgabe.append(Praefix(praefixe,i,trivials+trivials3,Id=multiid))
                        trivials=[]
                        praefixe=[]
                        coefi=coefi[:-1]
                        continue
                        ...
                        for i in coefi[-1]:
                            ...
                            if i in trivials2.keys():
                                for i1 in trivials2[i]:
                                    ...
                                    trivials3.append(i1)
                                    trivials2[i]=trivials2[i][1:]
                            ausgabe.append(Suffix(temp[0],i,trivials+trivials3,Id=multiid))
                        trivials=[]
                        praefixe=[]
                        coefi=coefi[:-1]
                        x=temp[1]
                        ...
                        if temp[0]:
                            if len(temp)==3 and temp[2]=="zucker":
                                ...
                                for i in zuckerends:
                                    ...
                                    if "yl"==ze:
                                        if not coefi:
                                            coefi=[[]]
                                        if len(coefi[-1])==1:
                                            if coefi[-1][0].__class__==str and "g" in coefi[-1][0]:
                                                ylstelle=coefi[-1][0]
                                            coefi=coefi[:-1]
                                        else:
                                            ylstelle=1
                                    ...
                                    if "os"==ze:
                                        for i in zuckerends:
                                            ...
                                            zuckerausgabe.append(Praefix("hydroxy",gw,Id=multiid))
                                        if ul2==[]:
                                            vausgabe.append(Praefix("oxo",1,Id=multiid))
                                        else:
                                            vausgabe.append(Praefix("hydroxy",1,Id=multiid))
                                    if "ofuran"==ze:
                                        gw2=5
                                        zuckerausgabe.append(Praefix("oxa",5,Id=multiid))
                                        trivials.append("cyclo")
                                    if "opyran"==ze:
                                        gw2=6
                                        zuckerausgabe.append(Praefix("oxa",6,Id=multiid))
                                        trivials.append("cyclo")
                                    if "o-tri"==ze:
                                        gw=3
                                    if "o-tetr"==ze:
                                    ...
                                    if "ul"==ze:
                                        if i[1]==1:
                                            i[1]=2
                                        zuckerausgabe.append(Praefix("oxo",i[1],Id=multiid))
                                        ul2.append(i[1])
                                    if "itol"==ze:
                                        zuckerausgabe.append(Praefix("hydroxy",1,Id=multiid))
                                        zuckerausgabe.append(Praefix("hydroxy",gw,Id=multiid))
                                    if "odiald"==ze:
                                        vausgabe.append(Praefix("oxo",1,Id=multiid))
                                        zuckerausgabe.append(Praefix("oxo",gw,Id=multiid))
                                    if "onsaeure"==ze:
                                        zuckerausgabe.append(Praefix("hydroxy",1,Id=multiid))
                                        zuckerausgabe.append(Praefix("oxo",1,Id=multiid))
                                        zuckerausgabe.append(Praefix("hydroxy",gw,Id=multiid))
                                    if "uronsaeure"==ze:
                                        zuckerausgabe.append(Praefix("oxo",1,Id=multiid))
                                        zuckerausgabe.append(Praefix("hydroxy",1,Id=multiid))
                                        zuckerausgabe.append(Praefix("carboxy",gw,Id=multiid))
                                if "l*-" in  trivials:
                                    h=-1
                                else :
                                    h=1
                                ...
                                for i in zuckerinformation[0]:
                                    if ul+i in ul2:
                                        ul+=1
                                    zuckerausgabe.append(Praefix("hydroxy",ul+i,[zuckerinformation[0][i]*h],Id=multiid))
                                ...
                                for i2 in range(len(zuckerpraefix2)):
                                    ...
                                    for i in zuckerpraefix[0]:
                                        if ul+i in ul2:
                                            ul+=1
                                        zuckerausgabe.append(Praefix("hydroxy",ul+i,[zuckerpraefix[0][i]*h],Id=multiid))
                                    ul+=max(zuckerinformation[0].keys())
                                cyclogw=1
                                cyclogwch=True
                                if not "cyclo" in trivials:
                                    ...
                                    for i in vausgabe:
                                        if i.Name=="oxo":
                                            i.Name="hydroxy"
                                            i.Beschreibung=["O",[0,1,0],["H",[0,1,0]]]
                                            if coefi and coefi[-1] and coefi[-1][0]=="3g":
                                                alphaoderbeta=3
                                            else:
                                                alphaoderbeta=2
                                            if alphaoderbeta==3:
                                                if "l*-" in  trivials:
                                                    i.Bezeichnungen.append(1)
                                                else:
                                                    i.Bezeichnungen.append(-1)
                                            else:
                                                if "l*-" in  trivials:
                                ...
                                if gw2!=0:
                                    for i in zuckerausgabe:
                                        if i.Name=="hydroxy" and i.Lokant==cyclogw+gw2-1:
                                            zuckerausgabe.remove(i)
                                            break
                                    if gw-gw2+2-cyclogw>0:
                                        bz=-1
                                        if "l*-" in  trivials:
                                            bz=1
                                        zuckerausgabe3=System(gw-gw2+2-cyclogw,{1:["yl"]},[bz],Lokant=gw2-1,Id=multiid)
                                        za3=True
                                else:
                                    gw2=oldcopy.copy(gw)
                                if "cyclo" in trivials:
                                    for i in zuckerausgabe:
                                        if i.Lokant>gw2-2 and i.Name!="oxa":
                                            i.Lokant=i.Lokant-gw2+2
                                            zuckerausgabe3.unterordnen(i)
                                        else:
                                ...
                                if ylstelle:
                                    ...
                                    for i in range(len(sonderausgabe)):
                                        if sonderausgabe[i].Lokant==ylstelle:
                                            if sonderausgabe[i].Name=="hydroxy":
                                                if 1 in sonderausgabe[i].Bezeichnungen:
                                                    trivials.append(1)
                                                elif -1 in sonderausgabe[i].Bezeichnungen:
                                                    trivials.append(-1)
                                                del sonderausgabe[i]
                                                break
                                            elif sonderausgabe[i].__class__==System and "hydroxy" in sonderausgabe[i].Untergruppen:
                                    ...
                                    sonderausgabe=[]
                                ausgabe.append(System(gw2,endungen,trivials,sicherzugeordnete=sonderausgabe,Id=multiid))
                                trivials=[]
                                x=temp[1]
                            else:
                            ...
                            for i in ("(","bis(","tris(","tetrakis(","s(","kis("):
                                if x.startswith(i):
                                    if coefi:
                                        if (len(coefi[-1])!=3 and i=="s(") or (len(coefi[-1])<4 and i=="kis("):
                                            break
                                    elif i in ("s(","kis("):
                                        break
                                    multi=["(","bis(","tris(","tetrakis(","s(","kis("].index(x[:len(i)])+1
                                    if multi>4:
                                        multi=len(coefi[-1])
                                    x=x[len(i):]
                                    temp=analyse(x)
                                    if temp[1]==")":
                                        if "poly" in trivials:
                                            temp[0].Bezeichnungen.append("poly")
                                        multiid+=1
                                        if coefi:
                                            for i in coefi[-1]:
                                                temp[0].Lokant=i
                                    ...
                                    endstemp=ends(temp[1][1:],[],[])
                                    if endstemp[0]!=[]:
                                        if endstemp[0][0][0] in ("ylen","yliden","ylidin","ylin","yl","thioyl","oyl","oxy","azo","azano","azeno"):
                                            temp[0].Endungen.dazu(endstemp[0][0][0],endstemp[0][0][1])
                                            temp=(temp[0],")"+endstemp[1])
                                    multiid+=1
                                    if not coefi or len(coefi[-1])!=multi:
                                        coefi=[range(multi)]
                                        temp[0].Bezeichnungen.append("coeftest")
                                    elif "coeftest" in temp[0].Bezeichnungen:
                                        while "coeftest" in temp[0].Bezeichnungen:
                                            temp[0].Bezeichnungen.remove("coeftest")
                                    for i in coefi[-1]:
                                        temp2=oldcopy.deepcopy(temp)
                                        temp2[0].id=multiid
                                        temp2[0].Lokant=i
                                        ausgabe.append(temp2[0])
                                    coefi=coefi[:-1]
                                    x=temp[1]
                            ...
                            if x[0]=="!":
                                ...
                                if "?" not in ausgabe:
                                    ...
                                    trivialindex=-2
                                    while ausgabe[trivialindex]!="?":
                                        if type(ausgabe[trivialindex])!=str and ausgabe[trivialindex].id==ausgabe[-1].id:
                                            trivialindex-=1
                                        else:
                                            substituentenantrivial.append(ausgabe.pop(trivialindex))
                                    ausgabe[-1].unterordnen(substituentenantrivial)
                                    ausgabe[-1].Bezeichnungen.append("trivial")
                                continue
    ...
    if x:
        ausgabe.reverse()
        ausgabe=ausgabeordner(ausgabe)
        for i in trivials2.keys():
            for i2 in trivials2[i]:
                ausgabe.Bezeichnungen.append(str([i2,i]))
        ausgabe.Bezeichnungen+=trivialsend
        return ausgabe,x
...
def hauptgruppe(i):
    if len(i)==2 and (len(i[0])==3 or (len(i[0])>3 and "trivial" in i[0][2])):
        return True
def acene(anzahl):
    system=System(6,{1:["en"],3:["en","o("],5:["en"]},["cyclo"],Lokant=3)
    for i in range(anzahl-1):
        i2=System(6,{1:["en"],3:["en","o("],5:["en"]},["cyclo"],Lokant=3)
        i2.unterordnen(system)
        system=i2
    system.Lokant=1
    system.Endungen[3].remove("o(")
    return system
def ausgabeordner(ausgabe):
    global csubs
    praelist=[]
    fuell=[]
    while ausgabe:
        i=ausgabe.pop(0)
        if i.__class__==Suffix:
            i,ausgabe,h=Hzaehl(i,ausgabe)
            praelist.append(i)
            continue
        elif i.__class__==System:
            ausgabes=i
            if ausgabes.inEndungen("acen"):
                ausgabes=acene(i.Laenge)
            for i1 in praelist:
                for i2 in ("+++","++","+"):
                    if i2 in i1.Beschreibung:
                        endung=["yl","ylen","ylin"][i2.count("+")-1]
                        break
                for i2 in ausgabes.Endungen:
                    if endung in ausgabes.Endungen[i2]:
                        ausgabes.Endungen[i2].remove(endung)
                        i1.Lokant=i2
                        i1.Bezeichnungen.remove("coeftest")
                        ausgabes.unterordnen(i1)
                        break
                while ausgabe[0].__class__==ausgabes.__class__ and ausgabe[0].id==ausgabes.id:
                    ausgabes.Untergruppen[-1].unterordnen(ausgabe.pop(0))
                verknuepfungen=str(ausgabes.Untergruppen[-1].Beschreibung).count("+")
                i2=0
                while ausgabe and verknuepfungen<len(ausgabes.Untergruppen[-1].Untergruppen)+1:
                    if i2>=len(ausgabe):
                        for i in range(verknuepfungen-len(ausgabes.Untergruppen[-1].Untergruppen)-1):
                            ausgabes.Untergruppen[-1].unterordnen(Praefix("hydro"))
                        break
                    if ausgabe[i2].__class__==ausgabes.__class__:
                        ausgabes.Untergruppen[-1].unterordnen(ausgabe.pop(i2))
                    else:
                        i2+=1
            #Verbundnamen
            while ausgabe and ausgabe[0].__class__==str:
                fuell.append(ausgabe.pop(0))
            multi=["bi","tris","tetrakis","pentakis"]
            for i1 in multi:
                if i1 in i.Bezeichnungen:
                    i.Bezeichnungen.remove(i1)
                    aehnlich=System(Laenge=i.Laenge,Endungen={},sicherzugeordnete=i.Untergruppen,Bezeichnungen=i.Bezeichnungen,Id=0)
                    for i2 in i.Endungen:
                        for i3 in i.Endungen[i2]:
                            aehnlich.Endungen.dazu(i3,i2)
                    for i2 in range(multi.index(i1)+1):
                        i.unterordnen(aehnlich)
                    i.Radikalloesch()
            if ausgabe and ausgabe[0].__class__==System:
                if not ausgabe[0].isRadikal() and "cyclo" in ausgabe[0].Bezeichnungen and not "cyclo" in ausgabes.Bezeichnungen:
                    i1=ausgabe.pop(0)
                    ausgabes.Endungen.dazu("yl",ausgabes.Laenge)
                    for i2 in range(len(ausgabe)):
                        if ausgabe[i2].__class__ in (Praefix,System) and ausgabe[i2].Lokant.__class__==str and "g" in ausgabe[i2].Lokant:
                            ausgabe[i2].Lokant=eval(ausgabe[i2].Lokant[:-1])
                            ausgabes.unterordnen(ausgabeordner([ausgabe[i2]]))
                            ausgabe[i2]="-"
                    while "-" in ausgabe:
                        ausgabe.remove("-")
                    ausgabes.Lokant=i.Lokant
                    i1.unterordnen(oldcopy.copy(ausgabes))
                    ausgabes=i1
            break
    #Stammgruppe=ausgabes
    last2=ausgabes
    while ausgabe:
        i=ausgabe.pop(0)
        last=last2
        ...
        if i.__class__==str:
            ...
            fuell=[]
        #Vorbereitungen getroffen, Zuordnung beginnt
        if i.__class__==Suffix:
            for i2 in ausgabe:
                if i2.__class__==System:
                    i2.unterordnen(i)
            continue
        elif i.__class__==Praefix:
            if "+" in i.Beschreibung:
                for i1 in range(i.Beschreibung.count("+")):
                    ausgabes2=System(ausgabes.Laenge,ausgabes.Endungen,ausgabes.Bezeichnungen,Id=0,Lokant=ausgabes.Lokant)
                    i.unterordnen(ausgabes2)
            if i.csub:
                ausgabes.unterordnen(i)
                continue
            if not vergleich(i,last,ausgabe):
                i,ausgabe,potential1=Hzaehl(i,ausgabe)
                if potential1>0 and not fuell:
                    i,ausgabe=Gruppen(i,ausgabe,potential1,[System])
            if i.Lokant.__class__==list:
                referenz=i.Lokant.count("'")
                i.Lokant=eval(i.Lokant.replace("'",""))
                i2=0
                for i1 in ausgabes.Untergruppen:
                    if i1.__class__==System and not i1.inEndungen("o("):
                        i2+=1
                        if i2==referenz:
                            while "coeftest" in i.Bezeichnungen:
                                i.Bezeichnungen.remove("coeftest")
                            i1.unterordnen(i)
                            break
                    elif i1.__class__ in (Suffix,Praefix):
                        for i3 in i1.Untergruppen:
                            if i3.__class__==System and not i3.inEndungen("o("):
                                i2+=1
                                if i2==referenz:
                                    i3.unterordnen(i)
                                    break
            if i.Lokant.__class__==str:
                referenz=i.Lokant.count("'")
                i.Lokant=eval(i.Lokant.replace("'",""))
                i2=0
                for i1 in ausgabes.Untergruppen:
            ...
            if ausgabes.inEndungen("acen"):
                ausgabes=acene(i.Laenge)
            if not vergleich(i,last,ausgabe):
                if not(ausgabe[0].__class__==System and ausgabe[0].inEndungen("o(")) and not i.inEndungen("o("):
                    i,ausgabe=Gruppen(i,ausgabe,-1,(System,Praefix))
            multi=["bi","tris","tetrakis","pentakis"]
            for i1 in multi:
                if i1 in i.Bezeichnungen:
                    i.Bezeichnungen.remove(i1)
                    ...
                    for i2 in range(multi.index(i1)+1):
                        i.unterordnen(aehnlich)
                    i.Radikalloesch()
            i2=i.inEndungen("o(",False)
            if i2:
                if len(i2)==1:
                    i.Lokant=i2[0]
                elif len(i2):
                    i.Lokant=i2[0]
                    i.Endungen.dazu([i2[-1],len(i2)+1],"o(")
                    for i3 in i2[1:]:
                        i.Endungen[i3].remove("o(")
        ausgabes.unterordnen(i)
    #aus dem alten ausgabeordner uebernommene Anweisungen
    if ausgabes.__class__==System:
        for i in ausgabes.Untergruppen:
            if i.__class__==System:
                if "zucker" in str(i.Endungen):
                    zuckerstelle=i.Lokant
                    for i2 in range(len(ausgabes.Untergruppen)):
                        if ausgabes.Untergruppen[i2].__class__==Praefix and ausgabes.Untergruppen[i2].Name=="hydroxy":
                            if ausgabes.Untergruppen[i2].Lokant==zuckerstelle:
                                del ausgabes.Untergruppen[i2]
                                break
    if ausgabes.__class__==System:
        for i2 in range(len(ausgabes.Bezeichnungen)):
            if type(ausgabes.Bezeichnungen[i2])==list:
                for i in range(len(ausgabes.Bezeichnungen)):
                    if ausgabes.Bezeichnungen[i] in ("(s)","(r)"):
                        ausgabes.Bezeichnungen[i]=str([ausgabes.Bezeichnungen[i],ausgabes.Bezeichnungen[i2].pop(0)])
                del ausgabes.Bezeichnungen[i2]
                break
    ausgabes=neueLokanten(ausgabes)
    ausgabes=cycloreparierer(ausgabes)
    ausgabes=induzierregler(ausgabes)
        #Ende der aus dem alten Ausgabeordner uebernommenden Anweisungen
    return ausgabes
def neueLokanten(ausgabes):
    for i in ausgabes.Untergruppen:
        if i.__class__==System:
            for i1 in i.Endungen.keys():
                if "o(" in i.Endungen[i1]:
                    if "coeftest" in i.Bezeichnungen:
                        i.Bezeichnungen.remove("coeftest")
                    for i2 in range(len(ausgabes.Untergruppen)):
                        if ausgabes.Untergruppen[i2].__class__==str:
                            continue
                        if ausgabes.Untergruppen[i2].id==i.id or (ausgabes.Untergruppen[i2].__class__==Praefix and ausgabes.Untergruppen[i2].csub) or (ausgabes.Untergruppen[i2].__class__==System and ausgabes.Untergruppen[i2].inEndungen("o(")) or ausgabes.Untergruppen[i2].Lokant.__class__==str:
                            continue
                        lokant=oldcopy.copy(ausgabes.Untergruppen[i2].Lokant)+i1
                        if lokant>ausgabes.Laenge-1:
                            ausgabes.Untergruppen[i2].Lokant=i.Laenge-(lokant-ausgabes.Laenge+1)
                            i.unterordnen(ausgabes.Untergruppen[i2])
                            ausgabes.Untergruppen[i2]="-"
                        else:
                            ausgabes.Untergruppen[i2].Lokant=lokant%ausgabes.Laenge+1
                    while "-" in ausgabes.Untergruppen:
                        ausgabes.Untergruppen.remove("-")
                    neuesendungsdictionary={}
                    neuesendungsdictionary2={}
                    for i2 in ausgabes.Endungen.keys():
                        for i3 in ["en","in","o(","an"]:
                            if i3 in ausgabes.Endungen[i2]:
                                neuesendungsdictionary2=dazu(i3,i2,neuesendungsdictionary2)
                                ausgabes.Endungen[i2].remove(i3)
                        neuesendungsdictionary[i2+i1+1]=ausgabes.Endungen[i2]
                    for i2 in neuesendungsdictionary:
                        if i2>ausgabes.Laenge:
                            for i3 in neuesendungsdictionary[i2]:
                                i.Endungen.dazu(i3,i2-ausgabes.Laenge+3)
                    for i2 in neuesendungsdictionary2:
                        for i3 in neuesendungsdictionary2[i2]:
                            neuesendungsdictionary=dazu(i3,i2,neuesendungsdictionary)
                    ausgabes.Endungen=Endungsdic({})
                    ausgabes.Endungen.dazu(neuesendungsdictionary)
                    break
        for i2 in range(len(i.Untergruppen)):
            i.Untergruppen[i2]=neueLokanten(i.Untergruppen[i2])
    return ausgabes
def Hzaehl(i,ausgabe,plus=False):
    psyn2=summenformel2(i.Beschreibung)
    potential1=0
    #Wasserstoffe für etwaige Substitutionen werden gesucht
    if "H" in psyn2.keys():
        potential1=psyn2["H"]
        if i.Beschreibung[0]=="H":
            potential1-=1
    else:
        potential1=0
    psynlist={}
    for i2 in ausgabe:
        if i2.__class__ in (Praefix,System) and i2.Lokant in psyn2:
            if i2.Lokant not in psynlist:
                psynlist[i2.Lokant]=potenziell(i.Beschreibung,i2.Lokant)
            if psynlist[i2.Lokant]==0:
                continue
            i.unterordnen(ausgabeordner([i2]))
            ausgabe[ausgabe.index(i2)]="##"
            psynlist[i2.Lokant]-=1
            potential1-=1
    while "##" in ausgabe:
        ausgabe.remove("##")
    return i,ausgabe,potential1
def Gruppen(i,ausgabe,potential1,klassen):
    for i2 in ausgabe:
        if i2.__class__ in klassen:
            if "coeftest"in i.Bezeichnungen:
                if not "coeftest" in i2.Bezeichnungen:
                    i.Lokant=i2.Lokant
                    i.Bezeichnungen.remove("coeftest")
                i.unterordnen(ausgabeordner([i2]))
                ausgabe.pop(0)
...
def induzierregler(ausgabes):
    for i in ausgabes.Untergruppen:
        if i.__class__==System:
            i=cycloreparierer(i)
            h=Hholer(i.Bezeichnungen)
            if h:
                h,i.Bezeichnungen=h
        elif i.__class__==Praefix:
            h=Hholer(i.Bezeichnungen)
            ...
            continue
        if h and "cyclo" in ausgabes.Bezeichnungen:
            ausgabes.Endungen=Endungsdic({})
            ausgabes.Endungen.dazu(enSchubser(ausgabes.Endungen,h))
    return ausgabes
...
def Hholer(x2):
    for i in x2:
        ...
        if i[0]=="H":
            x2.remove(i)
            return eval(i[1:]),x2
def cycloreparierer(ausgabes):
    for i in ausgabes.Untergruppen:
        if i.__class__==System:
            if "cyclo" in i.Bezeichnungen:
                    ylstelle=0
                    for i3 in i.Endungen.keys():
                        for i2 in ("ylen","ylin","yliden","ylidin","yl","thioyl","oyl","oxy","azo","azano","azeno"):
                            if i2 in i.Endungen[i3]:
                                i.Endungen[i3].remove(i2)
                                i.Endungen.dazu("eigentlichyl",i3)
                                i.Endungen.dazu(i2,1)
                                ylstelle=i3
                    if ylstelle!=0:
                        for i3 in i.Untergruppen:
                            i3.Lokant=(i3.Lokant-ylstelle)%i.Laenge+1
                        for i3 in range(len(i.Bezeichnungen)):
                            if i.Bezeichnungen[i3][0]=="[":
                                i.Bezeichnungen[i3]=eval(i.Bezeichnungen[i3])
                                i.Bezeichnungen[i3][1]=(i.Bezeichnungen[i3][1]-ylstelle)%i.Laenge+1
                                i.Bezeichnungen[i3]=str(i.Bezeichnungen[i3])
            i=cycloreparierer(i)
    return ausgabes
def radikaltester(x):
    if type(x)==type([]):
        for ii in ("ylen","ylin","yliden","ylidin","yl","thioyl","oyl","oxy","azo","o(","azano","azeno"):
            if ii in x:
                return True
    elif type(x)==type({}):
        for ii in ("ylen","ylin","yliden","ylidin","yl","thioyl","oyl","oxy","azo","o(","azano","azeno"):
...
def vergleich(y,x,z):
    if not z:
        return False
    if ((type(z[0])!=str and z[0].id==y.id) or (type(x)!=str and x.id==y.id)):
        return True
...
def potenziell(x,y):
    ...
    if x[0]==y:
        ...
        beding=False
    for i in x[1:]:
        if type(i[0]) in (int,float) or i.__class__==str:
            continue
        if i[0]=="H":
            p+=1
        else:
...
def Oersetzer(x,y):
    if x[0]=="O":
        ...
        return x,True
    for i in range(len(x[1:])):
        x[i],a=Oersetzer(x[i],y)
        if a:
            return x,True
    return x,False
def Hersetzer(x,y,stamm,such="H"):
    for i in range(len(x[1:])):
        if x[i+1]=="+":
            x[i+1]=y
            return x,True
        if x[i+1][0].__class__!=str:
            continue
        if x[i+1][0]==such:
            if stamm and x[0]!=stamm:
                continue
            x[i+1]=y
            return x,True
        elif len(x[i+1])>2:
            x[i+1],a=Hersetzer(x[i+1],y,stamm,such)
            if a:
                return x,True
    return x,False
def analysezusyntax(x,check=False,endzusatz={}):
    global cis,trans,csubs,mi,annelierungswinkel,fehlerapp,cyc2
    if x.__class__ in(Praefix,Suffix):
        if x.__class__==Suffix:
            x.praefixconvert()
        if x.csub:
            for i in x.Bezeichnungen:
                if i.__class__==str and i.startswith("lambda"):
                    x.Beschreibung[1]=eval(i[7:-1])
        else:
            for i in praesub:
                if i in x.Bezeichnungen:
                    x.Beschreibung,a=Oersetzer(x.Beschreibung,praesub[i])
            syntaxlist=[]
            for i in range(len(x.Untergruppen)):
                syntaxlist.append(analysezusyntax(x.Untergruppen[i])[1])
                if x.Untergruppen[i].Lokant in atomlist.keys():
                    stamm=x.Untergruppen[i].Lokant
                else:
                    stamm=False
                x.Beschreibung=Hersetzer(x.Beschreibung,"~",stamm)[0]
            for i in syntaxlist:
                x.Beschreibung=Hersetzer(x.Beschreibung,i,stamm,"~")[0]
        per=0
        lr=0
        if "per" in x.Bezeichnungen:
            per=1
        if "(s)" in x.Bezeichnungen:
            lr="links"
        elif "(r)" in x.Bezeichnungen:
            lr="rechts"
        if "(z)" in x.Bezeichnungen:
            x.Beschreibung[1][2]="cis"+str(cis/2)
            cis+=1
        elif "(e)" in x.Bezeichnungen:
            x.Beschreibung[1][2]="trans"+str(trans/2)
            trans+=1
        if 1 in x.Bezeichnungen and not x.csub:
            x.Beschreibung[1].append("1")
        elif -1 in x.Bezeichnungen and not x.csub:
            x.Beschreibung[1].append("0")
        return [x.Lokant,x.Beschreibung,per,lr,[]]
    else:
...
##        6/0
        per=["H",[0,1,0]]
        substituenten={}
        if x.Lokant.__class__ in(int,str):
            stelle=x.Lokant
        else:
            stelle=0
        triarg={}
        ...
        secarg={}
        oka=False
        for i in x.Endungen.keys():
            for a in x.Endungen[i]:
                if a=="o(":
                    for i3 in (i-1,i):
                        if i3 in endzusatz:
                            oka=True
        ...
        dehydrolist=[]
        for i in x.Untergruppen:
            if i.__class__==Praefix and i.Name=="dehydro":
                if not "coeftest" in i.Bezeichnungen:
                    dehydrolist.append(i.Lokant)
                    x.Untergruppen[x.Untergruppen.index(i)]=""
        while "" in x.Untergruppen:
            x.Untergruppen.remove("")
        for i in x.Untergruppen:
            if i.__class__==Praefix and i.Name=="dehydro":
                if "coeftest" in i.Bezeichnungen:
                    for i2 in range(x.Laenge):
                        if not (i2-2)%x.Laenge+1 in secarg and not (i2-1)%x.Laenge+1 in secarg and not (i2)%x.Laenge+1 in secarg and i2 not in dehydrolist:
                            dehydrolist.append(i2)
                    x.Untergruppen[x.Untergruppen.index(i)]=""
        dehydrolist.sort()
        while dehydrolist:
            i=dehydrolist.pop(0)
            if i+1 in dehydrolist:
                secarg[i]=[0,2,0]
                dehydrolist.remove(i+1)
            elif dehydrolist:
                i2=dehydrolist.pop(0)
                endungen2={i:[0,2,0]}
                for i3 in secarg.keys():
                    if i2>=i3 and i3>=i:
                        endungen2[i3+2]=secarg[i3]
                        del secarg[i3]
                for i3 in endungen2.keys()[:-1]:
                    secarg[i3]=endungen2[i3]
        while "" in x.Untergruppen:
            x.Untergruppen.remove("")
        for i in x.Endungen.keys():
                for a in x.Endungen[i]:
                    if i>x.Laenge and a in ("en","in"):
                        Endungslokantenfehler.append("'"+a+"'")
                        x.Endungen[i].remove(a)
                        continue
                    if i in Hsammler:
                        Hsammlerzahl+=1
                    elif i+1 in Hsammler:
                        Hsammlerzahl+=2
                    if a in ("azen","en","azeno") and not "o(" in a:
                        encount2=0
                        cistrans=0
                        for i2 in x.Bezeichnungen:
                            if i2 in ("(z)","(e)"):
                                i2={"(e)":"trans","(z)":"cis"}[i2]
                                if encount2==encount:
                                ...
                                encount2+=1
                        if oka:
                            secarg[i+1+Hsammlerzahl]=[0,2,cistrans]
                        else:
                            secarg[i+Hsammlerzahl]=[0,2,cistrans]
                        encount+=1
                    if "in" == a:
                        print Hsammler
                        secarg[i]=[0,3,0,0]
                        if i in Hsammler:
                            ...
                            Hsammlerzahl+=2
                        if oka:
                            secarg[i+1+Hsammlerzahl]=[0,3,0,0]
                        else:
                            secarg[i+Hsammlerzahl]=[0,3,0,0]
        hydrolist=[]
        for i in x.Untergruppen:
            if i.__class__==Praefix and i.Name=="hydro":
                if not "coeftest" in i.Bezeichnungen:
                    hydrolist.append(i.Lokant)
                    x.Untergruppen[x.Untergruppen.index(i)]=""
        while "" in x.Untergruppen:
            x.Untergruppen.remove("")
        for i in x.Untergruppen:
            if i.__class__==Praefix and i.Name=="hydro":
                if "coeftest" in i.Bezeichnungen:
                    for i2 in secarg:
                        if secarg[i2][1]>1 and i2 not in hydrolist:
                            hydrolist.append(i2)
                        elif i2+1 not in hydrolist:
                            hydrolist.append(i2+1)
                        else:
                            continue
                        x.Untergruppen[x.Untergruppen.index(i)]=""
                        break
        ...
        while hydrolist:
            ...
            if i+1 in hydrolist:
                ...
                if secarg[i][1]==1:
                    del secarg[i]
                hydrolist.remove(i+1)
            elif hydrolist:
                i2=hydrolist.pop(0)
                endungen2={}
                for i3 in secarg.keys():
                    if i2>=i3 and i3>=i:
                        endungen2[i3+1]=secarg[i3]
                        del secarg[i3]
                for i3 in endungen2.keys()[:-1]:
                    secarg[i3]=endungen2[i3]
        for i in x.Untergruppen:
            if not i:
                continue
            if "poly" in x.Bezeichnungen:
                if i.Lokant=="2g":
                    erstes_polymerende=analysezusyntax(i)[1]
                    continue
                if i.Lokant=="25g":
                    zweites_polymerende=analysezusyntax(i)[1]
                    continue
            if i.__class__==System and not i.isRadikal:
                #Hier die Verbundnamenomenklatur einarbeiten
                i.Endungen.dazu("yl",1)
            if "coeftest" in i.Bezeichnungen:
                i.Bezeichnungen.remove("coeftest")
                coeftestwuerdig.append(i)
                continue
            a=analysezusyntax(i,endzusatz=secarg)
            if "+" in str(a[1]):
            ...
            if type(a[1][1])==int:
                ...
                if a[4]!=[]:
                    #a[0]=a[4][0][0]
                    substituenten=dazu(["@0"+a[4][0][2]+a[4][0][1],[-annelierungsbindung,a[4][0][3],0]],a[0],substituenten)
                    if "@" in a[1][2][0]:
                        a[1]=a[1][3]
                    else:
                        a[1]=a[1][2]
                    #a[1][1].append(str((a[0]+1)%2))
                    a[1][1].append("1")
                    a[1][1][0]=annelierungsbindung
                    a[0]=(a[0])%x.Laenge+1
                    a[1][1][2]=10*pi
                    a[1][1].append("n"+str(a[4][0][2]))
                if a[2]!=0:
                    per=a[1]
                else:
        ...
        if 0 in secarg.keys():
            del secarg[0]
        for i in x.Endungen.keys():
            if i not in (x.Laenge,1) or "cyclo" in x.Bezeichnungen or "bicyclo" in x.Bezeichnungen:
                carbo=True
            else:
                carbo=False
            for a2 in range(len(x.Endungen[i])):
                a=x.Endungen[i][a2]
                if a not in ("sil","silaz","silox","azan","azen","german","stann","plumb","stib"):
                    if type(i)!=str and i>x.Laenge:
                        Endungslokantenfehler.append("'"+a+"'")
                        continue
                if a in ("oat","at"):
                    if i in substituenten:
                        ...
                        del substituenten[i][-1]
                         ...
                         a2=analysezusyntax(coeftestwuerdig[0])[1]
                ...
                if "sultam"==a:
                    cyclo=1
                    nochdran=[i,x.Laenge-i,{i+2:[["O",[0,2,0]],["O",[0,2,0]]]},{i+1:["N",3],i+2:["S",6]}]
                ...
                if "lactam"==a:
                    ...
                    nochdran=[i,x.Laenge-i,{},{i+1:["N",3]}]
                    x.Laenge=i+1
                if "sulton"==a:
                    cyclo=1
                    nochdran=[i,x.Laenge-i,{i+2:[["O",[0,2,0]],["O",[0,2,0]]]},{i+1:["O",3],i+2:["S",6]}]
                    x.Laenge=i+2
                if a in ("olid","lacton"):
                    if i==1:
                        i2=x.Laenge
                    ...
                    cyclo=1
                    substituenten=dazu(["O",[0,2,0]],1,substituenten)
                    nochdran=[i2,x.Laenge-i2,{},{i2+1:["O",2]}]
                    x.Laenge=i2+1
                if "sil" == a:
                    zentral=["Si",4]
                if "german" == a:
                ...
                if "o(" == a:
                    ...
                    if 1 in secarg:
                        ...
                        ylbindung=[0,1,0]
                    if x.Laenge-1 in secarg:
                        b=secarg[x.Laenge-1]
                        b[0]="s1"
                    else:
                        b=["s1",1,0]
                    substituenten=dazu(["@0"+str(cyc2)+"C",b],x.Laenge-1,substituenten)
                    lweg=True
                    andereseite="C"
                    if x.Laenge-1 in substituenten:
                        for i2 in substituenten[x.Laenge-1]:
                            if i2 in csubs.keys():
                                andereseite=csubs[i2][0]
                    oklatur.append([i,andereseite,str(cyc2),b[1]])
                    cyc2+=1
                if "azo" == a:
                    ylstelle=i
                    ylbindung=[0,1,0]
                ...
                if "amid" == a:
                    ...
                    if amidaminolist:
                        ...
                        if substituenten[i]!=[]:
                         ...
                         a2=analysezusyntax(coeftestwuerdig.pop(0))[1]
                    if carbo:
                ...
                if "oyl" == a:
                    ylstelle=i
                    ylbindung=[0,1,0]
                    if (i==1 or i==x.Laenge) and not "cyclo" in x.Bezeichnungen:
                        substituenten=dazu(["O",[0,2,0]],i,substituenten)
                    else:
                ...
                if "saeureanhydrid" == a:
                    subs=True
                    for sny in x.Untergruppen:
                        if sny.__class__==System and sny.inEndungen("saeure"):
                            subs=False
                            x.Untergruppen.remove(sny)
                            for i in sny.Endungen:
                                if "saeure" in snyEndungen[i]:
                                    sny.Endungen[i].remove("saeure")
                                    if "cyclo" in sny.Bezeichnungen:
                                        sny.Endungen[i].append("eigentlichyl")
                                    sny.Endungen.dazu("oyl",i)
                                    break
                            x2=analysezusyntax(sny)[1]
                            break
                    if subs:
                        x2=oldcopy.copy(x)
                        x2.Endungen[i].remove("saeureanhydrid")
                        x2.Endungen.dazu("oyl",i)
                        if "cyclo" in x2.Bezeichnungen:
                            x2.Endungen.dazu("eigentlichyl",i)
                        x2=analysezusyntax(x2)[1]
                        x.Endungen[i].remove("oyl")
                    x2=["O",[0,1,0],x2]
                    if carbo:
        ...
        for i in coeftestwuerdig:
            if x.__class__==System and not x.isRadikal():
                #auch hier die Verbundnamenklatur einarbeiten
                x.Endungen.dazu("yl",1)
            if "o-" in i.Bezeichnungen:
                ctwlist=[1,2,6]
            elif "m-" in i.Bezeichnungen:
                ctwlist=[1,3,5]
            elif "p-" in i.Bezeichnungen:
                ctwlist=[1,4]
            else:
                ctwlist=range(1,x.Laenge+1)
            if last==0:
                last=ctwlist[0]
            for i4 in ctwlist[ctwlist.index(last):]+ctwlist[:ctwlist.index(last)+1]:
                ...
                if "cyclo" in x.Bezeichnungen and i4 in (1,x.Bezeichnungen):
                    summe=summe-1
                if summe>0:
        ...
        for i in range(len(multiplikation)):
            alkan=lineare_aliphaten(x.Laenge,secarg,eval("substituenten"+str(i+2)),triarg,per,zentral,cyclo,stelle=2)
            alkan[1]=[0,1,0]
        ...
        if ylstelle!=0:
            ...
            if oklatur:
                if x.Laenge-1 in triarg:
                    oklatur[-1][1]=triarg[x.Laenge-1][0]
            for i in x.Bezeichnungen:
                if i.__class__==str and i[0]=="[":
                    i=eval(i)
            ...
            print substituenten,secarg,ylbindung,ylstelle
            alkan=Chiralitaetszentrenordner(lineare_aliphaten(x.Laenge,secarg,substituenten,triarg,per,zentral,cyclo,stelle=2,lweg=lweg,ylstelle3=ylstelle3))
            i=1
            if ylbindung==[0,3,0]:
                alkan=alkan[:3]
                alkan[1]=[0,3,0]
                ...
                alkan[1]=ylbindung
            umtausch=[]
            if "thio" in x.Bezeichnungen:
                umtausch.append("S")
            if "seleno" in x.Bezeichnungen:
                umtausch.append("Se")
            if "telluro" in x.Bezeichnungen:
                umtausch.append("Te")
            if "pollonio" in x.Bezeichnungen:
                umtausch.append("Po")
            alkan=str(alkan)
            for um in umtausch:
                alkan=str(alkan)
            ...
            alkan=eval(alkan)
            if bruecke!="":
                if bruecke=="['N',[0,1,0],['N',[0,2,0],+]]":
                    alkan[1][2]=pi
                alkan=eval(bruecke.replace("+",str(alkan)))
            if len(alkan)>1:
                for i in range(2,len(alkan)):
                    if alkan[i][1][0]:
                        alkan[i][1][0]=pi-alkan[i][1][0]/2
            if "poly" in x.Bezeichnungen:
                ...
                return alkan
            if 1 in x.Bezeichnungen:
                alkan[1].append("1")
            elif -1 in x.Bezeichnungen:
                alkan[1].append("0")
            return [stelle,alkan,per2,lr,oklatur]
        for i in range(len(x.Bezeichnungen)):
            if x.Bezeichnungen[i][:8]=="bicyclo[":
                bicyclo=split(x.Bezeichnungen[i][8:-1],".")
                for i in range(len(bicyclo)):
                    bicyclo[i]=int(bicyclo[i])
                x,Laenge=bicyclo[0]+2
                zweiter_ring=[bicyclo[1]+2,{},{},{}]
                for i in secarg:
                    if i>x.Laenge:
                        zweiter_ring[1].__init__({i-x.Laenge:secarg[i]})
                for i in substituenten.keys():
                    if i>x.Laenge:
                        zweiter_ring[2].__init__({i-x.Laenge+1:substituenten[i]})
                for i in triarg:
                    if i>=x.Laenge:
                        zweiter_ring[3].__init__({i-x.Laenge+1:triarg[i]})
        ...
        for i in chzentren:
            if i[0] in triarg.keys():
                ...
                triarg[i[0]]=[zentral[0]+"~"+i[1],zentral[1]]
        alkan=lineare_aliphaten(x.Laenge,secarg,substituenten,triarg,per,zentral,cyclo,lweg=lweg)
        for i in chzentren:
            alkan=str(alkan)
...
def lineare_aliphaten(x,y,sonderwuensche={},Csub={},per=["H",[0,1,0]],zentral=["C",4],cyclo=0,stelle=1,lweg=False,ylstelle3=False):
    ...
    if cyclo!=0:
        ...
        if type(s)==type(1.3):
            s1=radians(109.48)
            s2=[s for i in range(x)]
        elif type(s)==list:
            s1=0
            s2=s
        elif len(s)==3:
            s1=s[0]
            s2=[s[1] for i in range(x)]
        else:
            s1=s[0]
            s2=0
        if stelle==3:
        ...
        if x in y:
            ...
            b=1
        if x in sonderwuensche:
            sonderwuensche[x].insert(0,["@"+str(cyc)+cychilfe,[s1,b,s2.pop(),0]])
        else:
            sonderwuensche[x]=[["@"+str(cyc)+cychilfe,[s1,b,s2.pop(),0]]]
        ...
        if 1 in sonderwuensche:
            sonderwuensche[1].insert(0,["@"+str(cyc)+cychilfe,[s1,b,0,0]])
        else:
            sonderwuensche[1]=[["@"+str(cyc)+cychilfe,[s1,b,0,0]]]
        cyc+=1
        for i in y.keys():
            y[i][0]=s1
            y[i][2]=s2[i]
    ...
    while i!=x:
        ...
        while x-i in sonderwuensche:
            ...
            if a==len(alkan)-1:
                ...
                sonderwuensche[x-i]=sonderwuensche[x-i][1:]
        for a in range(2,len(alkan)):
            if "1" in alkan[a][1] or "0" in alkan[a][1]:
                if "1" in alkan[a][1]:
                    alkan[a][1].remove("1")
                    if stelle==2:
...
def Chiralitaetszentrenordner(x,lr=0,stern="*"):
    if not "*" in x[0] and ((len(x)>2 and "@" in x[1][0]) or (len(x)>3 and "@" in x[3][0])):
        if len(x[2])>2 and len(x[2][1])>3:
            if "3" in x[2][2][0]:
                x[2][2][1][2]=pi
    x=str(x)
    if not "*" in x:
...
def CIPprioritaetenordner(syntax,lr):
    ...
    for i in range(1,len(syntax)):
        ...
        reihenfolge.append([[_ordnungszahl(acs),syntax2[i][1][1]]])
    for i in range(len(reihenfolge)):
        while reihenfolge.count(reihenfolge[i])!=1:
            ...
            for n in bearbeitungsbedarf:
                ...
                reihenfolge[n].append(geradedazu[-1])
    ...
    for i in range(2):
        for i2 in range(len(syntax2[2:])):
            if not mitzahlen(syntax[i2+2][0]):
                if not i2+2 in nichtkonst:
                    nichtkonst.append(i2+2)
        if len(nichtkonst)<2:
            for i2 in range(len(syntax2[2:])):
                if not (len(syntax2[i2+2][1])>3 and syntax2[i2+2][1][3]==1):
                    if not i2+2 in nichtkonst:
                        nichtkonst.append(i2+2)
                        break
    nichtkonst=nichtkonst[:2]
    if lr=="links":
...
def alizyklen(x,y,sub):
    ...
    y3=[y3[-1]]+y3+[y3[0]]
    for i in range(1,x+1):
        if y3[i+1]==1 and y3[i]==1 and y3[i-1]==1 and not (i==0 and y3[x]!=1):
...
##    gamma=pi/3.+2.*diff_angle(gamma,vector(0,1,0))
    a= annaeherung(x+1,False,False,False,False,360)#,180)
    return a
            ...
            if xcoef=="":
                ...
                for i in greek2:
                    if x.startswith(lower(i)):
                        s1c1.append(str(greek2.index(i)+2)+"g")
                        x=x[len(i):]
                        h=1
                        break
                if h==0 and s1c1==[]:
...
def aloescher(xi):
    if xi!="":
        if xi[0]=="a" and not orstartswith(xi,["al","amid","an","azano","azan","azeno","azen","acen","azo"]):
            return xi[1:]
    return xi
def ends(x,coefi,trivials,zucker=False):
    if coefi and type(coefi[-1])==list:
        coefi=coefi[-1]
    end1=("an","en","in","oxy","azo","silaz","silox","plumb","german","stann","stib","sil","enozonid","azano","azan","azeno","azen","acen")
    end2=("olid","sultam","lacton","sulton","lactam","lacton","nitril","o(","ylidin","ylin","ylen","yliden","yl","ol","thiol","on","aldehyd","al","thial","saeureanhydrid","carbonsaeure","saeure","oat","at","azo","oxy","amid","oyl","thioyl","carbonyl","ester")
    if zucker:
    ...
    while t==0:
        ...
        if not x.startswith("diald"):
            ...
            if temp2[1][0]=="a":
                for i in end1+end2:
                    if temp2[1].startswith(i):
                        break
                    elif temp2[1][1:].startswith(i):
                        temp2[1]=temp2[1][1:]
                        break
                else:
                    temp2=[]
        ...
        for i in end1:
            if temp[1].startswith(i):
                ...
                t=0
                break
        if x!="" and x[0]=="a" and not orstartswith(x,["al","an","at","amid","azo"]):
            x=x[1:]
            t=0
            aloesch=True
    t=0
    while t==0:
        ...
        for i in end2:
            if temp[1].startswith(i):
                if i=="al" and temp[1].startswith("alkohol"):
                    continue
                if i=="ylen":
                ...
                if i=="o(":
                    ...
                    x=temp[1][temp[1].index(")")+1:]
                    while a[0]!=")":
                        if a[0] in letters:
                            endcoef.append(letters.index(a[0])+1)
                            a=a[1:]
                        elif a[0] in digits:
                            a1=coefs(a)
                            a=a1[1]
    ...
    while h==1:
        ...
        if anz==0:
            ...
            if temp[0]!="":
                ...
                anz=1
        for i in csubs.keys():
            if i in ("fluora","chlora","broma","ioda"):
                continue
            if x.startswith(i[:-1]):
...
def Systeminfo(x,m=""):
    if x.__class__==Praefix:
        print m+"Praefix"
        print m+" Name          :",x.Name
        print m+" Beschreibung  :",x.Beschreibung
        print m+" Csub          :",x.csub
    elif x.__class__==Suffix:
        print m+"Suffix"
        print m+" Name          :",x.Name
        print m+" Beschreibung  :",x.Beschreibung
    elif x.__class__==System:
        print m+"System"
        print m+" Laenge        :",x.Laenge
        print m+" Endungen      :",x.Endungen
    else:
        return
    print m+" Bezeichnungen :",x.Bezeichnungen
    print m+" Lokant        :",x.Lokant
    if x.Untergruppen:
        print m+" Untergruppen  :"
        for i in x.Untergruppen:
            Systeminfo(i,m+"   ")
nurbeschreibung("cyclohexan")
#print Komplexanalyse("dichloro(triphenylphosphino)platinat(IV)").Bauen()
#nurbeschreibung("cis-1-chlor-2-fluorethen")
VP=False
def colorcompare(a):
    a2=[]
...
class MyMenu(wx.Frame):
    def __init__(self, parent, id,title,vergleich=False):
        ...
        ansicht.Append    (209,u"Bindungslänge",u"Berücksichtigung der Bindungslängen",wx.ITEM_CHECK)
        materialchoice=wx.Menu()
        materialchoice.Append(2101,"Ohne","Ohne Materialeffekte",wx.ITEM_RADIO)
        materialchoice.Append(2102,"Plastic","Plastikmaterialeffekt",wx.ITEM_RADIO)
        materialchoice.Append(2103,"Holz","Holzmaterialeffekt",wx.ITEM_RADIO)
        materialchoice.Append(2104,"Marmor","Marmormaterialeffekt",wx.ITEM_RADIO)
        self.Bind(wx.EVT_MENU,self.changematerial,id=2101)
        self.Bind(wx.EVT_MENU,self.changematerial,id=2102)
        self.Bind(wx.EVT_MENU,self.changematerial,id=2103)
        self.Bind(wx.EVT_MENU,self.changematerial,id=2104)
        ansicht.AppendMenu(210,"Material",materialchoice)
        ...
        if vergleich:
            ...
            self.slider11 = AtomSlides(self.panel,11,200,30, (13,136),"Atomproportion"    ,self.slider11neu,True)
            ...
            self.slider14 = AtomSlides(self.panel,14,36,10, (132,206),unicodestring    ,False,True)
            ...
            self.slider14 = AtomSlides(self.panel,14,36,10, (228,206),unicodestring    ,False)
    ...
    def zeichne(self,a,SMILES=False,vergleich=False):
        ...
        if SMILES:
            ...
            #self.bsch=Komplexanalyse(a2).Bauen()
            if self.bsch[0].__class__!=str:
                self.bsch=self.bsch[1]
                self.bsch[1]=["@1"+self.bsch[0],[0,self.bsch[1][1],0]]
            for i in range(len(a)):
                if a[i] in letters:
...
##        demo=wx.MessageDialog(self.panel,u"Diese Version von Chython - digitales Molekülmodell ist auschließlich für Demonstrationszwecke bestimmt.\nEs handelt sich um die Demoversion Chython 0.9 in der Ansichtsversion für den Tim Hotfilter.\nVervielfältigung des Programms ist nicht gestattet.\n\nFolgende Einschränkungen wurden in dieser Version eingerichtet:\n - Nach jedem Zeichnen wird diese Meldung angezeigt.\n - Alle Atome außer Kohlenstoff, Wasserstoff, Sauerstoff und Stickstoff werden durch Schwefel ersetzt.\n - Die Transparenz sowie der Fullscreenmodus sind nicht darstellbar\n\nWir bitten um Ihr Verständnis\n\nStefan Mühlbauer, Ben Heuer 27.02.2010",caption="Chython Demoversion - Informationen",style=wx.OK)
##        demo.ShowModal()
        ...
        if not self.evtmenusonoff[0]:
            ...
            if VP:
                if vergleich:
                    SetWindowPos( VP, HWND_TOPMOST,-5,-32,508,410, SWP_SHOWWINDOW)
                else:
                    SetWindowPos( VP, HWND_TOPMOST,-5,-32,594,720, SWP_SHOWWINDOW)
        ...
        if self.evtmenusonoff[1]:
            ...
            for i in range(len(self.sza[2])):
                ...
                if text=="xx":
                    continue
                label(text=text,pos=self.sza[1][i].pos,linecolor=self.sza[1][i].color,height=20)
    ...
    def changematerial(self,evt):
        if evt.GetId()==2101:
            ...
            self.material=materials.marble
        self.drawmaterial()
...
class MyApp(wx.App):
    def OnInit(self):
        self.frame=MyMenu(None, -1, 'Chython.py')
        self.frame.Show(True)
        return True
    def zeichne(self,a):
        self.frame.zeichne(a)
